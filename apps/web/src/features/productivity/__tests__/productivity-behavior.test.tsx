...
describe("Productivity Behavior", () => {
  it(
    "renders mastery + goals and refetches after creating goal",
    async () => {
      let listCalls = 0;
      const goals: Array<{
        id: string;
  ...
  it("shows backend and network errors", async () => {
    server.use(
      http.get("*/analysis/mastery", () =>
        HttpResponse.json({ message: "Invalid mastery request" }, { status: 400 }),
      ),
    );
    render(<ProductivityPage />);
    await waitFor(() => {
    ...
    server.use(
      http.get("*/learning-goals", () => HttpResponse.error()),
    );
    render(<ProductivityPage />);
    await waitFor(() => {
