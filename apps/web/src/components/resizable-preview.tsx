...
const ResizablePreviewContext =
  React.createContext<ResizablePreviewContextValue | null>(null);

function useResizablePreview() {
  const context = React.useContext(ResizablePreviewContext);
  if (!context) {
    throw new Error(
      "useResizablePreview must be used within a ResizablePreview",
    );
  }
  return context;
}
...
function ResizablePreviewProvider({
  ...
  const [breakpoint, setBreakpointState] =
    React.useState<ViewportBreakpoint | null>(defaultBreakpoint);
...
function ResizablePreview({
  ...
  return (
    <div
      ...
      className={cn(
        "group/resizable-preview flex h-full min-w-0 flex-col",
        className,
      )}
      {...props}
    />
...
function ResizablePreviewContent({
  className,
  ...props
}: React.ComponentProps<"div">) {
  return (
...
function ResizablePreviewHandle({
  className,
  ...props
}: React.ComponentProps<"div">) {
  const {
      ...
      if (e.key === "ArrowLeft" || e.key === "ArrowRight") {
        ...
        animationFrameRef.current = requestAnimationFrame(() => {
          ...
          if (e.key === "ArrowRight") {
            const boundary =
              boundaryRef.current ??
              resizableContainerRef.current?.parentElement;
            if (boundary) {
