...
class FakeConceptRepository implements ConceptRepository {
  ...
  getConcepts(): ConceptEntity[] {
    return [...this.concepts.values()];
  }
}

describe('Graph Stability', () => {
  it('removes non-document nodes during sync', async () => {
    const conceptRepository = new FakeConceptRepository();
    const documentRepository = new FakeDocumentRepository();
    const useCase = new BuildGraphUseCase(
      conceptRepository,
      documentRepository,
    );

    await conceptRepository.findOrCreate('random-concept');
